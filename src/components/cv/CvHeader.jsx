...
function CvHeader() {
    const { editing, data, update, onToggleToolbar } = useCvEdit();
        ...
        <div className="bg-gray-200 a4-height h-64 px-8 flex flex-col">
            <div className="py-8 self-center">
                <img
                    ...
                    className="rounded-full w-48 p-1 border border-blue-800 border-2 select-none"
                    onDoubleClick={onToggleToolbar}
                />
            </div>
            <div className="text-2xl font-bold">
                <EditableField value={header.name} onChange={v => update("header.name", v)} />
            </div>
            <div className="text-lg mt-2">
                <EditableField value={header.title} onChange={v => update("header.title", v)} />
            </div>
            <div className="text-md mt-8">
                ...
                <EditableField value={header.contact.address[0]} onChange={v => update("header.contact.address[0]", v)} />
                <div className="ml-6">
                    <EditableField value={header.contact.address[1]} onChange={v => update("header.contact.address[1]", v)} />
                </div>
            </div>
